alter table cost drop foreign key FK_7tp1o00p8t18nd3b1e7a5a8a0;
alter table cost_amounts drop foreign key FK_hvk2kb9c3u2qmvagc59utxm7b;
...
drop table if exists cost_amounts;
...
create table cost (id integer not null auto_increment, unit varchar(255), planmetadata_id integer, primary key (id));
create table cost_amounts (cost_amounts_id integer not null, value double precision, currency varchar(255) not null, primary key (cost_amounts_id, currency));
...
create table plans (id varchar(255) not null, description varchar(255) not null, is_free bit, name varchar(255), plan_cred_id varchar(255), metadata_id integer, service_id varchar(255), primary key (id));
create table service_bindings (id varchar(255) not null, app_guid varchar(255) not null, instance_id varchar(255) not null, plan_id varchar(255) not null, service_id varchar(255) not null, service_binding_id varchar(255), primary key (id));
...
alter table cost add constraint FK_7tp1o00p8t18nd3b1e7a5a8a0 foreign key (planmetadata_id) references planmetadata (id);
alter table cost_amounts add constraint FK_hvk2kb9c3u2qmvagc59utxm7b foreign key (cost_amounts_id) references cost (id);
